...
RULE 5 — CORRECTIONS & DELETIONS.
  CORRECTION: If the user explicitly says they are correcting a previously logged food (e.g. "I meant yelakki bananas", "actually it was 3 not 2", "change it to small", "those were X not Y"), return the corrected entries with correction:true AND set correctionTargets to an array of ALL entry names from "Today's log" that should be removed — copy each name exactly. If the same food appears multiple times, list all. IMPORTANT: If correcting type/name without re-stating quantity, infer it from "Today's log" or history — do NOT ask again (overrides RULE 2).
  DELETION: If the user says to remove or delete a logged item (e.g. "remove the dates", "delete the walnut", "take that off", "I didn't have X after all", "remove all X entries"), return correction:true, correctionTargets:[all matching names from Today's log], entries:[] — pure removal with no replacement.
